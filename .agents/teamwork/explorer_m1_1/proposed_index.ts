  ...
  try {
    ...
    if (status === 'REJECTED') {
      ...
      if (booking.user.fcmToken) {
        try { await getMessaging().send({ token: booking.user.fcmToken, notification: { title: 'تم رفض حجزك ❌', body: `قام المالك بإلغاء حجزك في ${booking.court.name}.` } }); } catch (e) {}
      }
      ...
      if (booking.court.venue.owner.fcmToken) {
        try { await getMessaging().send({ token: booking.court.venue.owner.fcmToken, notification: { title: 'إلغاء حجز من اللاعب ❌', body: `قام اللاعب ${booking.user.name} بإلغاء حجزه في ${booking.court.name}.` } }); } catch (e) {}
      }
    }
